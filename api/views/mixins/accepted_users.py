...
class AcceptedUsersMixin:

    @action(detail=True, methods=['get'])
    def accepted_users(self, request: Request, pk) -> Response:
        instance: Playlist | Experience = self.get_object()
        qs: QuerySet
        # Query's accept this enables ordering reverse chronologically of accept creation.
        match instance:
            case Playlist():
                qs = PlaylistAccept.objects.filter(playlist=instance.id)
            case Experience():
                qs = ExperienceAccept.objects.filter(experience=instance.id)
        qs = qs.order_by('-created_at') \
            .prefetch_related('user')

        page_size = get_page_size_from_request(request, 20)
        paginator = AppPageNumberPagination(page_size=page_size)
        user_accepts_page = paginator.paginate_queryset(qs, request)
        user_ids = [user_accept.user.id for user_accept in user_accepts_page]
